import { UNI_HEME_MAI_PAIR } from './Constants'
...
export function getHEMEUSDRate(): BigDecimal {
  let pair = UniswapV2Pair.bind(Address.fromString(UNI_HEME_MAI_PAIR))
  ...
  if (reserve0.equals(BigDecimal.zero())) {
    log.debug('getHEMEUSDRate div {}', [reserve0.toString()])
    return BigDecimal.zero()
  }

  let hemeRate = reserve1.div(reserve0).div(BIG_DECIMAL_1E9)
  log.debug('HEME rate {}', [hemeRate.toString()])
...
export function getPairUSD(
  ...
  let ohm_value = toDecimal(lp_token_0, 9).times(getHEMEUSDRate())
